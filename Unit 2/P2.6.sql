SET SERVEROUTPUT ON;
DECLARE
    v_food NUMBER := 450; v_distance NUMBER := 6;
    v_rain BOOLEAN := TRUE; v_late BOOLEAN := FALSE; v_festival BOOLEAN := TRUE; v_peak BOOLEAN := FALSE; v_first BOOLEAN := TRUE;
...
BEGIN
    ...
    CASE WHEN v_late THEN v_late_s:=v_base*0.15; ELSE v_late_s:=0; END CASE;
    CASE WHEN v_festival THEN v_fest_s:=v_base*0.10; ELSE v_fest_s:=0; END CASE;
    CASE WHEN v_peak THEN v_peak_s:=v_base*0.10; ELSE v_peak_s:=0; END CASE;
